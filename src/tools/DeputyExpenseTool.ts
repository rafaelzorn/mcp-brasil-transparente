    ...
    this.server.registerTool(
      'get-deputy-expenses',
      {
        ...
        inputSchema: z.object({
          ...
          month: z
            .number()
            .optional()
            .describe('Mês para consulta'),
        }).refine(data => data.year !== undefined || data.month !== undefined),
        outputSchema: {
          data: z.array(
            z.object({
      ...
      async ({ deputyId, year, month }) => {
        try {
          ...
          return {
            content: [{ type: 'text', text: 'Ops... Ocorreu um erro ao buscar as despesas do deputado.' }],
            structuredContent: {data: []},
          }
        }
      }
