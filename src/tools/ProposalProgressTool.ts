...
export class ProposalProgressTool {
  ...
  private registerGetProposalProgressesTool(): void {
    this.server.registerTool(
        ...
        outputSchema: {
          data: z.array(
            z.object({
              ...
              descriptionSituation: z.string().describe('Descrição da situação da tramitação'),
      ...
      async ({ proposalId }) => {
        try {
          const proposalProgresses = await this.proposalProgressService.getProposalProgresses(
            proposalId,
          )
          ...
          return {
            content: [{ type: 'text', text: 'Ops... Ocorreu um erro ao buscar as tramitações da proposição.' }],
            structuredContent: {data: []},
          }
        }
      }
